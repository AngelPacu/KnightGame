...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        currentCol = jugadores[currentClientSocket].col;
        currentRow = jugadores[currentClientSocket].row;

        //Comprobar si el jugador esta vivo
        if(!isPlayerAlive(currentClientSocket)){
            //Enviar respuesta 'D' al cliente
            ...
            switch(whereIsGoingTo(currentClientSocket, packClient.command)){
                case ' ':
                    ...
                    //Enviar respuesta 'L' al cliente
                    send(client_socket, "L", sizeof(char), 0);
                    break;
                case 'P':
                    //Enviar respuesta 'P' al cliente
                    send(client_socket, "P", sizeof(char), 0);
                    break;
                case 'V':
                    //Actualizar posicion
                    setPosition(currentClientSocket, packClient.command);
                    //Enviar respuesta 'V' al cliente
                    send(client_socket, "V", sizeof(char), 0);
                    break;
                case 'T':
                    //Actualizar posicion
                    setPosition(currentClientSocket, packClient.command);
                    //Enviar respuesta 'T' al cliente
                    send(client_socket, "T", sizeof(char), 0);
                    //Restar una vida
                ...
                case 'S':
                    //Actualizar posicion
                    setPosition(currentClientSocket, packClient.command);
                    //Enviar respuesta 'S' al cliente
                    send(client_socket, "S", sizeof(char), 0);
                    break;
            }
        }
